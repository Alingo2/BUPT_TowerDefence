...
class Level_choose(cocos.menu.Menu):
    ...
    def level_1_callback(self):
        ...
        self.e_list.append([Enemy_1(), False])
    ...
    def update(self, dt):
        ...
        for enemy_1 in self.e_list:
            #print(self.e_list)
            if not enemy_1[1]:
                if self.coll_manager.they_collide(self.player_1, enemy_1[0]):
                    global block_1, block_1_R
                    block_1_R = True
                    enemy_1[0].auto_attack = True
                    if enemy_1[0].near_attack:
                        block_1 = True
                ...
                if self.count >= 7:
                    self.count = 0
                if self.coll_manager.they_collide(self.player_1.bullet, enemy_1[0]):
                    if self.count == 0:
                        self.count += 1
                        self.player_1.count = 5
                        print("hit")
                        if enemy_1[0].life <= 20:
                            enemy_1[0].life = 0
                            enemy_1[1] = True
                            scroller.remove(enemy_1[0])
                            del enemy_1
                        else:
                            enemy_1[0].life = enemy_1[0].life - 5
                            enemy_1[0].beheat = True
            else:
                block_1_R = False
                block_1 = False
